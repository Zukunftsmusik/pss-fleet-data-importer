...
def test_create(gdrive_file: GDriveFile, collection_file_db: CollectionFileDB, cancel_token: CancellationToken):
    queue_item = CollectionFileQueueItem(1, gdrive_file, collection_file_db, "/dev/null/", cancel_token)
    ...
    assert queue_item.download_file_path is None
...
def test_properties(
    ...
    queue_item.download_file_path = "/dev/null/file.json"
    assert queue_item.download_file_path == Path("/dev/null/file.json")
